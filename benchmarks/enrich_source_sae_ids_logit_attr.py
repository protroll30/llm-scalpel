"""
Fill per-row ``src_feature_ids`` from Layer-8 SAE features ranked by **logit attribution** on the **clean** prompt.

Unlike ``enrich_source_sae_ids.py`` (top-|f| magnitude), this script scores each latent with how much it
supports the **correct next-token logit** at the end of the prompt:

- ``--rank-by dot`` (default): use ``f ⊙ (∂logit/∂f)`` at the subject / metric position — same local linear
  attribution as ``discovery.attribution.feature_act_grad_scores`` / the circuit pruner.
- ``--rank-by grad``: rank by ``∂logit/∂f`` only (directional sensitivity toward raising the answer logit).

Position resolution matches ``enrich_source_sae_ids.py`` (``subject_pos`` first, then metric fields, else
fallback). The scalar traced through the model is ``logits[0, -1, answer_token_id]`` where ``answer_token_id``
is the **first GPT-2 token** of ``correct_answer``.

After enrichment, intervene with ``--mode set --value 5.0`` (and **without** large counterfactual scales).

Example::

  python benchmarks/enrich_source_sae_ids_logit_attr.py \\
...
from discovery.attribution import _feat_grad_corrupt_forward, feature_act_grad_scores
...
def _top_k_indices_by_scores(scores: torch.Tensor, k: int) -> list[int]:
    kk = min(int(k), int(scores.numel()))
    _, ix = torch.topk(scores, k=kk)
    return [int(i) for i in ix.cpu().tolist()]


def main() -> int:
    p = argparse.ArgumentParser(
        description="Write src_feature_ids = top Layer-8 latents by clean-prompt logit attribution to correct_answer."
    )
    ...
    p.add_argument(
        ...
        default="dot",
        choices=("dot", "grad"),
        help="dot: rank by f⊙∂logit/∂f; grad: rank by ∂logit/∂f (signed).",
    )
    p.add_argument(
        "--skip-dropped",
        action="store_true",
    ...
    p.add_argument(
        ...
        default="last",
        choices=("cli", "last"),
        help="When JSON has no subject/metric seq keys: last=-1 or --seq-pos.",
    )
    args = p.parse_args()
    ...
    for row in tqdm(pairs, desc="src_feature_ids_logit_attr"):
        ...
        try:
            ...
            def logits_to_answer_logit(logits: torch.Tensor) -> torch.Tensor:
                return logits[0, -1, tid]

            if str(args.rank_by) == "dot":
                scores_vec = feature_act_grad_scores(
                    model=model,
                    prompt=clean,
                    hook_name=hook_name,
                    encode_fn=encode_src,
                    decode_fn=decode_src,
                    logits_to_scalar_loss=logits_to_answer_logit,
                    seq_pos=pos_eff,
                    prepend_bos=bool(args.prepend_bos),
                    device=device,
                    forced_zero_indices=None,
                )
                ids = _top_k_indices_by_scores(scores_vec, k)
            else:
                _f, g, _pe, _ga = _feat_grad_corrupt_forward(
                    model=model,
                    prompt_corrupt=clean,
                    hook_name=hook_name,
                    encode_fn=encode_src,
                    decode_fn=decode_src,
                    logits_to_scalar_loss=logits_to_answer_logit,
                ...
                ids = _top_k_indices_by_scores(g, k)
    ...
    if isinstance(gm, dict):
        ...
        gm["src_feature_ids_enrichment"] = {
            "script": "benchmarks/enrich_source_sae_ids_logit_attr.py",
            "rule": "top_k_logit_attribution_layer8_on_clean_prompt",
            "rank_by": str(args.rank_by),
            "detail_dot": "top_k of f ⊙ ∂logit_answer/∂f at seq_pos",
            "detail_grad": "top_k of ∂logit_answer/∂f at seq_pos",
